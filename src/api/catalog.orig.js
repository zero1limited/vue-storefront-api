...
export default ({config, db}) => function (req, res, body) {
	...
	if(searchAdapter === 'fredhopper' && categoryId !== null) {
		...
		if(entityType === 'product') {
			...
			// We need to call Elasticsearch first unfortunately, because we need the fredhopper_category_id.
			...
			request({ // do the elasticsearch request
				...
				if (_resBody && _resBody.hits && _resBody.hits.hits) {
					...
					if (!fredhopperCategoryId) {
						console.log('WARNING: there was no fredhopperCategoryId with this request');
					} else {
						console.log('OK: there was a fredhopperCategoryId with this request');
					}
					...
					request({ // do the Fredhopper request
						...
						if (_resBody && _resBody.info) {
							...
							if (fredhopperProcessor) {
								...
								productProcessor.process(_resBody.hits.hits, groupId).then((result) => {
					...
					console.error('Initial Elasticsearch request has failed.');
				}
			});
		}
